...
char *end_word(char* str,char delim) {
	...
	int i = strlen(str) - 1; // gets last element of string
...
char** tokenize(char* str, char delim)
{
	...
	for(int i = 0; i < tokens; i++){
		new_string[i] = copy_str(first, strlen(str), delim);//copies string, starting with the first word, and so on
		first = end_word(last, delim); //gets the delimeter in order to start the new word and swap
		last = word_start(first, delim); //gets the first letter in order to get the next word
	}
	return new_string;
}
